...
<div class="border-nested my-32">
	<header class="flex-y-center bg-slate-950 p-4 text-slate-50">
		<h1 class="flex-x-1 text-6xl font-black tracking-tighter uppercase">
		...
		<nav class="flex-x-4 my-4 justify-center">
			{#each [{ name: 'About', href: '/' }, { name: 'Projects', href: '/projects' }, { name: 'Blog', href: '/blog' }] as { name, href }, i}
				<a {href} class="btn hover:-outline-offset-[6px]">
					{i}/ {name} -->
				</a>
			{/each}
		</nav>
	...
	<main>
		<div class="flex max-md:flex-col">
			...
			>
				...
				<p>
					I work mainly with front-end technologies like React and TypeScript, but I'm comfortable
					with backend languages like Python and PHP.
				</p>
		...
		{#if more}
			<div
				class="flex flex-row-reverse max-md:flex-col"
				in:slide={{ easing: linear, duration: 200 }}
			>
				<div
				...
				>
					...
					<p class="grow">
						Outside of coding, I'm a huge language nerd who's currently learning Ancient Greek. I
						also love board games, swimming, biking, and the occasional meditation retreat.
					</p>
...
<style lang="postcss">
	...
	main {
		...
		p {
			@apply indent-[1ch] leading-snug text-pretty hyphens-auto;
